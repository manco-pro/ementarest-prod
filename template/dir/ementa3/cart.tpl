...
<div class="container" style="margin-top: 3rem; padding: 1.8rem;">
    <div class="row">
        <div class="col-12 bg-dark bg-opacity-75 w-100 h-100 px-3 pt-3 pb-1 mb-3 rounded-3 msnisert">
            <p class="fs-6 fw-normal text-white initmsn"><strong>{$stCART}</strong> {$stMENSAJE_CART}</p>
        </div>
        {if isset($stPRODUCTOS)}
            <form id="Formcart">
                <!--Item-->
                {foreach from=$stPRODUCTOS item=producto key=key }
                    <div class="col-12 bg-white bg-opacity-50 p-2 mb-3" style="border-radius: 0.3rem;" id="DIV_{$producto.id}">
                        <div class="row">
                            ...
                            <div class="col col-sm-3 col-lg-2">
                                ...
                                <div class="d-flex flex-row flex-nowrap justify-content-around align-items-baseline h-100 pt-5">
                                    <i class="fa-solid fa-circle-minus fs-4 m-0 align-middle" id="Menos_{$producto.id}"
                                        style="color: #cccccc;"></i>
                                    <p class="fs-6 fw-bold" id="Cantidad_{$producto.id}">{$producto.cantidad}</p>
                                    <i class="fa-solid fa-circle-plus fs-4 m-0" id="Mas_{$producto.id}"
                                        style="color: #fa3434"></i>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-12">
                        ...
                        <div class="d-grid gap-2 mt-3" id="botones_pedido">
                            <button id="QR" class="btn text-white fw-bold botones_pedido"
                                style="background-color: #279EFF;" type="button" data-bs-toggle="modal"
                                data-bs-target="#obsprod">{$stBOTON1}
                            </button>
                            ...
                            {if isset($stMESA)}
                                ...
                                {if $stMESA_ENABLED=='enabled'}
                                    <button class="btn btn-success botones_pedido" id="Pedir" type="button"
                                        data-bs-toggle="modal" data-bs-target="#obsprod">{$stBOTON2}</button>
                                {/if}
                            {/if}
                        </div>
                        {/if}
